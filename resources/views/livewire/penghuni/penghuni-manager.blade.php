<div class="p-6">
    ...
    <div class="mb-4 p-4 bg-white rounded-lg shadow-sm">
        <h2 class="text-sm font-semibold text-gray-700 mb-1">Link Form Registrasi Penghuni</h2>
        <p class="text-xs text-gray-500 mb-3">Bagikan link ini ke calon penghuni kos Anda</p>
        <div class="flex items-center space-x-2">
            <input id="form-link" type="text" value="{{ url('/public/daftar-penghuni') }}" readonly
                class="flex-1 px-2 py-1 text-xs border rounded bg-gray-50 text-gray-600 focus:outline-none">
            <button onclick="navigator.clipboard.writeText(document.getElementById('form-link').value)"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($penghunis as $penghuni)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $penghuni->kamar?->nomor_kamar ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $penghuni->tanggal_masuk?->format('d/m/Y') ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if($penghuni->status === 'menunggu_verifikasi')
                                        <button wire:click="openVerifyModal({{ $penghuni->id }})" class="text-green-600 hover:text-green-900" title="Verifikasi">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                        </button>
                                    @endif
                                    <button wire:click="edit({{ $penghuni->id }})" class="text-blue-600 hover:text-blue-900" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>
                                    <button wire:click="openDeleteModal({{ $penghuni->id }})"
            ...
            <form wire:submit.prevent="save">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select wire:model.live="selectedBuildingForm" class="w-full px-3 py-2 border rounded-lg">
                    ...
                    <div>
                        ...
                        <select wire:model.live="selectedFloorForm" class="w-full px-3 py-2 border rounded-lg"
                            {{ !$selectedBuildingForm ? 'disabled' : '' }}>
                            <option value="">-- Pilih Lantai --</option>
                            @foreach($floorsForForm as $f)
                    ...
                    <div>
                        ...
                        <select wire:model="room_id" class="w-full px-3 py-2 border rounded-lg"
                            {{ !$selectedFloorForm ? 'disabled' : '' }}>
                            <option value="">-- Pilih Kamar --</option>
                            @foreach($roomsForForm as $r)
    ...
    <div class="fixed inset-0 backdrop-blur-sm bg-black/30 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
            ...
            <div class="mb-6">
                <p class="text-gray-600">Apakah Anda yakin ingin menghapus penghuni <strong>{{ $deleteNama }}</strong>?</p>
