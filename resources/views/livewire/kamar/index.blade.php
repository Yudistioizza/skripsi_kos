                        ...
                        @foreach($rooms as $room)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm space-x-3">
                                    <button wire:click="editRoom({{ $room->id }})" class="text-indigo-600 hover:text-indigo-900 font-medium">Edit</button>
                                    <button wire:click="deleteRoom({{ $room->id }})" onclick="return confirm('Yakin ingin menghapus kamar ini?')" class="text-red-600 hover:text-red-900 font-medium">Hapus</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <flux:modal wire:model="showRoomForm" heading="Form Kamar">
        <div class="space-y-4">
            <div>
                ...
                <select id="room_building_id" wire:model="room_building_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                    <option value="">Pilih Gedung</option>
                    @foreach($buildings as $building)
                        <option value="{{ $building->id }}">{{ $building->nama }}</option>
                    @endforeach
